...
| `pr-screenshots` | Before opening/updating a PR whose diff touches frontend/UI files (views, components, stylesheets, JSX/TSX/Vue/Svelte templates) — capture before/after screenshots of the affected screens with whatever already renders the app (dev server, e2e/browser-test setup, component-preview build), crop and annotate what changed, and attach them to the PR description/comment rather than leaving them only in chat. Skips cleanly when the diff has no rendered surface or the project has no way to render UI locally. Pairs with, but doesn't replace, a normal code review. |
