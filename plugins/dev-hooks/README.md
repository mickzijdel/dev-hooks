...
| `PostToolUse` (`Read`) | `hidden-text-reminder.sh` | When a file Claude just read contains Unicode steganography used to smuggle invisible prompt-injection text past a human reviewer while an LLM still parses it — Unicode Tag characters (`U+E0000`–`U+E007F`, the "ASCII smuggling" range) or a run of 6+ zero-width characters (`U+200B`/`U+200C`/`U+200D`/`U+2060`/`U+FEFF`) — feed a correction back (exit 2, **every occurrence**) telling Claude to inspect the flagged content before treating it as instructions. Deliberately narrow: it catches invisible-character steganography only, not visually-hidden CSS (`font-size:0`, color-on-color, `display:none`), which needs style computation this hook can't do reliably. Opt out with `DEV_HOOKS_HIDDEN_TEXT=false`. |
...
- Hooks require `jq` (used to parse hook input) and, for `a11y-reminder.sh`,
  `debug-leftover-reminder.sh`, `error-swallow-reminder.sh`, `hidden-text-reminder.sh`,
  `inline-svg-reminder.sh`, `memory-reminder.sh`, `missing-test-reminder.sh`,
  `review-reminder.sh`, `secret-plaintext-reminder.sh`, `sql-injection-reminder.sh`,
  `todo-leftover-reminder.sh`, and `verify-work.sh`, `python3`.
