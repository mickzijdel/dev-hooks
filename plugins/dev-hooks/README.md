...
| `data-before-design` | About to build, restyle or lay out anything that displays stored data — a card, panel, detail view, dashboard, table, report, profile page or summary strip. Two gates before any markup: **profile the field** (non-null count, distinct values, length min/mean/max, coverage across records, cardinality per parent, date presence, vocabulary) and state what you found in numbers — a field that is NULL on every row, always true, or 50 characters long changes what you should build — then **mock several options** from real records (the busiest, the typical and the empty one), say what each optimises for, and let the person choose. Paired with the `data-before-design-reminder` hook; complements `design-process` (aesthetics), `dataviz` (chart colour and form) and `popovers-tooltips` (floating UI mechanics). |
