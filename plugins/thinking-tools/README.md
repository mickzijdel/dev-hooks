...
| `agent-handoff` | Starting a new session to continue work a previous agent or session started, or coordinating parallel agents. Provides the plan.md format and handoff protocol so sessions can resume without re-explaining context — pairs with `dev-hooks`' `plan-reminder.sh` Stop hook. |
