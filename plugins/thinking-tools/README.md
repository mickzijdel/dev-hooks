...
| `grill-me` | Before merging a non-trivial implementation — dispatches parallel adversarial subagents (Bug Hunter, Security Auditor, Edge-Case Seeker, Performance Skeptic) to attack the code from independent angles, then synthesizes findings ranked by severity. Not for style (use `code-simplifier`) or idea critique (use `board`). |
