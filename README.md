...
| [`writing`](plugins/writing/) | Writing/content skills: `github-readme`, `humanizer`, `readability` (adapted from [Nate Berkopec's dotfiles](https://github.com/nateberkopec/dotfiles)) and `voice-profile` (match a person's writing voice from a profile of their own rules) — plus `readme-reminder`, `voice-intent-reminder`, and `voice-reminder` hooks that audit READMEs, nudge applying a voice profile before writing, and check prose against it on write. |
