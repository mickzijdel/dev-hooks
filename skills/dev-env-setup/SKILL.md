...
- **`mise.toml`** — tools pinned (`hk`, `pkl`, stack tool, `gitleaks`, `node` for jscpd),
  `[settings] lockfile = true`, and the `[env]` version stamp `DEV_ENV_VERSION = "12"`.
...
Only **uv** is checker-enforced for Python repos (see "The standard (v12)"); the rest are recommended
